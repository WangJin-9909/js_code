...
;(function ($, window, document) {
    ...
    PointCaptcha.prototype = {
                ...
                img.onload = function () {
                    $.ajax({
                        ...
                        success: function (res) {
                            ...
                            //对字符串进行转顺序，转成数组
                            var  seqArr = _this.apphaToNum(sequence);
                            for (var i = 0; i < seqArr.length; i++){
                                console.log("arr = " + seqArr[i]);
                            }

                            // 加载完成开始绘制
                            if (!_this.options.useCanvas) {
                                // _this.drawImage_NoCanvas(img);
                                _this.drawImageOnCanvas(img, seqArr);
                                // _this.drawImageOnCanvas(img, sequence); 以前10个以内字符串的方式
                            } else {
                                if (img.complete) {
                                    //       _this.drawImage(img);
        ...
        apphaToNum: function (alpha) {
            var length = alpha.length;
            var result = new Array(length);
            for (var i = 0; i < length; i++) {
                if (alpha.charAt(i) < 10) {
                    result[i] = alpha.charAt(i);
                }
                else {
        ...
        drawImageOnCanvas: function (img, sequence) {
            //图片块的数量，目前根据顺序数组的大小来决定
            var length = sequence.length;
            var pixelX = 300 / (length / 2);
            console.log("单块图片的X长度： " + pixelX);
            //准备canvas环境
            ...
            for (var i = 0; i < length; i++) {
                //des 根据sequence决定
                var des_x = Math.floor(sequence[i] / 2) * 30;
                //var des_x = Math.floor(sequence.charAt(i) / 2) * 30; 以前10个以内字符串的方式
                var des_y = (sequence[i] % 2) * 100;
                // var des_y = (sequence.charAt(i) % 2) * 100; 以前10个以内字符串的方式
                //src，根据当前 i 决定
                var src_x = Math.floor(i / 2) * 30;
                var src_y = (i % 2) * 100;
                context.drawImage(img, src_x, src_y, 30, 100, des_x, des_y, 30, 100);
            }
